...
def falseNegativeSUMAlltech2222(vul_data_array,CVEsAttack,techniquesID,arrayPositive ,arrayNegative,CvesNotAttack, Threeshold
    ...
    dataframeResultsForallModels = pd.concat([dataframeResultsForallModels, pd.DataFrame({'TechID':[techniquesID],'modelName':[modelName],'LintersectM':[LintersectM],'L_M':[L_M],'M':[M],'M_L':[M_L],'L_Sum':[L_Sum],'Jaccard':[jaccard]})], ignore_index=True)
...
def checkCVEUsingAllTech():
    ...
    dataframeResultsForallModels = pd.DataFrame(columns=['TechID','LintersectM','L_M','M','M_L','L_Sum','Jaccard'])
    ...
    for mod in SentenceTransformersModels:
        ...
        # num_train_samples = int(0.8 * len(tech_dict))
        # num_test_samples = int(0.2 * len(tech_dict))
        # num_validation_samples = len(tech_dict) - num_train_samples - num_test_samples
        # print(num_train_samples, num_test_samples, num_validation_samples)
        # items = list(tech_dict.items())
        # random.shuffle(items)
        # tech_dict = dict(items)

        # # Slice the shuffled list to get the desired number of samples for each set
        # train_data = dict(items[:num_train_samples])
        # test_data = dict(items[num_train_samples:num_train_samples + num_test_samples])
        # validation_data = dict(items[-num_validation_samples:])
        # trainDict = {}
        # trainText = []
        for Threeshold in [0.50]:
            df = df.iloc[0:0]
            dfRes = df.iloc[0:0]
            countT = 0
            ...
            for key, value in tech_dict.items():
                ...
                AttackTPsum,    AttackTNsum,    AttackFPsum,     AttackFNsum , dataframeResultsForallModels= falseNegativeSUMAlltech2222(vul_data_array,trainAndTestSetCVEs,key,arrayPositive , arrayNegative,CvesNotAttack, Threeshold, AttackTPsum,    AttackTNsum,    AttackFPsum,     AttackFNsum,dataframeResultsForallModels, model_parts[1])
            ...
            print(mod)
        dataframeResultsForallModels.to_excel(f"./Results/AllModelsJaccardNew2Re.xlsx", index=False)
        dataframeResults.to_excel(f"./Results/RResultsNewRe.xlsx", index=False)
        dfRes.to_excel(f"./Results/{model_parts[1]}NewRe.xlsx", index=False)
