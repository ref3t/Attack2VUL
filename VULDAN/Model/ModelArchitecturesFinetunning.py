...
def checkCVEUsingAllTech():
    global df
    global dfRes
    dataframeResults = pd.DataFrame(columns=['Model','precision','Recall','F1','Time'])
    ...
    # Assuming readTechWithNegative() is a function that returns a dictionary of techniques
    tech_dict = readTechWithNegative()

    # Extracting the keys (techniques) from the dictionary
    techniques = list(tech_dict.keys())

    # Splitting data into 80% training and 20% temporary sets
    train_techniques, temp_techniques = train_test_split(techniques, test_size=0.2, random_state=42)

    # Splitting the temporary set into 50% validation and 50% test sets (10% each of the original data)
    val_techniques, test_techniques = train_test_split(temp_techniques, test_size=0.5, random_state=42)
    for mod in SentenceTransformersModels:
        ...
        for infoData in informationData:
            ...
            for Threeshold in [0.58]:
                df = df.iloc[0:0]
                dfRes = df.iloc[0:0]
                countT = 0
                print(str(Threeshold) + " Threshold")
                for key in train_techniques:
                    value = tech_dict[key]
                ...
                for key in train_techniques:
                    ...
                    AttackTPsum, AttackTNsum, AttackFPsum, AttackFNsum, dataframeResultsForallModels = falseNegativeSUMAlltech2222(
                        vul_data_array, trainAndTestSetCVEs, key, arrayPositive, arrayNegative, CvesNotAttack, Threeshold, 
                        AttackTPsum, AttackTNsum, AttackFPsum, AttackFNsum, dataframeResultsForallModels, model_parts[1]
                    )
            ...
            dataframeResults = pd.concat([dataframeResults, pd.DataFrame({'Model': [model_parts[1]], 'precision': [preci], 'Recall': [Recal], 'F1': [F1score], 'Time': [formatted_time]})], ignore_index=True)
            print(mod)
