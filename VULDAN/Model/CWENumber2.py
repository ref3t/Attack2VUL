...
Cwe108 = diverse_vul_weaknesses_set - attack_weaknesses_set

# print("Cwe108",len(Cwe108))
# # print(set(Cwe108)[0])
# Cwe108 = list(Cwe108)
# print("################################################################################ Full Connection Attack + DiverseVul ###################################################################")
# #this for full connection attack
df = pd.read_excel('./datasets/CVECWEWithDescriptionFinal.xlsx')

# CweFromFile = df['TechnqiueID'].unique()
# print("CweFromFile" , len(CweFromFile))
df['CWE'] = df['CWE'].astype(str).str.strip().str.replace("CWE-", "").str.lower()

filtered_df = df[df['CWE'].isin(Cwe108)]
print("filtered_df" , len(filtered_df))
cwetoCVE = filtered_df['CWE'].unique()
print("cwetoCVE" , len(cwetoCVE))
print(cwetoCVE)

CWE85 = cwetoCVE
CWE23 = list(set(Cwe108)-set(CWE85))

print("CWE85",CWE85)
print("CWE23",CWE23)

filtered_df = df[df['CWE'].isin(CWE85)]
cwetoCVE = filtered_df['CVEID'].unique()
print("CVE85" , len(cwetoCVE))

# print("CAPECIDIDsDiverseVulAttack" , len(TechnqiueIDsDiverseVulAttack))
# print(TechnqiueIDsDiverseVulAttack)
# filtered_df = df[df['TechnqiueID'].isin(["T1027", "T1027.009", "T1027.006"])]
# df['CWE'] = filtered_df['CWE']
# print(df['CWE'])

df = pd.read_excel('./datasets/diversevulN.xlsx')

df['cwe'] = df['cwe'].astype(str).str.strip().str.replace("cwe-", "").str.lower()
filtered_df = df[df['cwe'].isin(CWE85)]
print("filtered_df" , len(filtered_df))
cwetoCVE85 = filtered_df['func'].unique()
print("CWE85Function85" , len(cwetoCVE85))

filtered_df = df[df['cwe'].isin(CWE23)]
print("filtered_df" , len(filtered_df))
cwetoCVE23 = filtered_df['func'].unique()
print("CWE85Function23" , len(cwetoCVE23))

# print(len(set(cwetoCVE23).intersection(set(cwetoCVE85))))
...
# filtered_df = df[df['CWE'].isin(diverse_vul_weaknesses_set)]

# TechnqiueIDsDiverseVulAttack = filtered_df['TechnqiueID'].unique()
# print("CAPECIDIDsDiverseVulAttack" , len(TechnqiueIDsDiverseVulAttack))
# print(TechnqiueIDsDiverseVulAttack)
