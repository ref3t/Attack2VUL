...
def validate_cve_news_pair_entity(cve_description, news_content, threshold=0.58):
    ...
    if overlap:
        ...
        return refined_score >= 0.50, refined_score  # Retain if refined score is above 0.70

    return False, initial_score

def validate_within_cve_news_pair_entity(cve_description, news_content):
    """Validate if a CVE matches a news article based on similarity and keyword/entity overlap."""
    
    # Extract keywords and entities from both texts
    cve_keywords_entities = set(extract_keywords_entities(cve_description))
    news_keywords_entities = set(extract_keywords_entities(news_content))
    
    # Check for overlap between keywords/entities
    overlap = cve_keywords_entities.intersection(news_keywords_entities)
    
    # Apply a secondary validation if overlap exists
    if overlap:
        refined_score = calculate_similarity_score(" ".join(overlap), news_content)
        return refined_score >= 0.45, refined_score  # Retain if refined score is above 0.70
...
def check_between_the_existing_CVEs_using_entity(attack_cve_just_related_dict, attack_cve_related_dict):
    ...
    for news in attack_cve_just_related_dict:
        ...
        for cve_description in descriptions:
            is_valid, score = validate_cve_news_pair_entity(cve_description, attack_texts, threshold=0.58)
...
def check_between_the_existing_CVEs_using_entity2(attack_cve_just_related_dict, attack_cve_related_dict):
    count = 0
    dataframeResults = pd.DataFrame(columns=['Attack', 'CVEs', 'CommonCVEs', 'RelatedWithoutCommonCVEs', 'ScoreswithinCommonCVEsEntity', 'foundCVEs'])
    ...
    for news in attack_cve_just_related_dict:
        ...
        # Prepare DataFrame for related CVEs
        df_cveid = pd.DataFrame(list(attack_cve_just_related_dict[news].keys()), columns=['CVEID'])
        df_desc = pd.DataFrame(list(attack_cve_just_related_dict[news].values()), columns=['CVEDescription'])
        dataCve = pd.concat([df_cveid, df_desc], axis=1)
        ...
        for cve_description in descriptions:
            is_valid, score = validate_cve_news_pair_entity(cve_description, " ".join(found_Cve_descriptions), threshold=0.58)
            if is_valid:
                results.append((cve_description, score))

        # Further validation of unmentioned CVEs
        unmentioned_cve_descriptions = [desc for desc, _ in results if desc not in found_Cve_descriptions]
        for idx, cve_description in enumerate(unmentioned_cve_descriptions):
            is_related, refined_score = validate_cve_news_pair_entity(cve_description, " ".join(found_Cve_descriptions), threshold=0.70)
            if is_related:
                CVEs_related_using_entity.append(f'{CVEs_id[idx]}#{refined_score:.2f}')

        dataframeResults = pd.concat([dataframeResults, pd.DataFrame({
            'Attack': [news],
            'ScoreswithinCommonCVEsEntity': [CVEs_related_using_entity],
            'foundCVEs': [found_cve_ids]
        })], ignore_index=True)

        count += 1
        print(f'{count} news items processed')
...
def add_scores_to_related_cves(attack_cve_just_related_dict, attack_cve_related_dict):
    count = 0
    dataframeResults = pd.DataFrame(columns=['Attack','CVEs','CVEs with Score', 'len'])
    for news in attack_cve_just_related_dict:
        print (list(attack_cve_related_dict[news]))
        # if count == 2:
        #     break
        ...
        external_embedding = model.encode(attack_texts)
        similarities = cosine_similarity(external_embedding.reshape(1, -1), embeddings)[0] 
        top_indices = np.argsort(similarities)[-181000:][::-1]


        # Iterate over the top similar CVEs and append them to the list
        CVEs_related_using_entity = []
        for idx in top_indices:
            score = similarities[idx]  # Get the precomputed similarity score
            CVEs_related_using_entity.append(f'{CVEs_id[idx]}#{score:.2f}')
            print(f"CVE {CVEs_id[idx]}:")
            print("Description:", descriptions[idx])
            print("Score:", score)
            print("-" * 40)


        dataframeResults = pd.concat([dataframeResults, pd.DataFrame({'Attack':[news], 'CVEs':[sorted(set(CVEs_id))],'CVEs with Score': [sorted(set(CVEs_related_using_entity))], 'len': [len(CVEs_related_using_entity)]})], ignore_index=True)
        
        
    dataframeResults.to_excel(f"./Results/NewsResults/News/latest/AddScoreToNewsCVEs.xlsx", index=False)

def check_cves_found_in_news_to_related_fisrt_cves(attack_cve_just_related_dict, attack_cve_related_dict):
    count = 0
    dataframeResults = pd.DataFrame(columns=['Attack','CVEs','CVEsScorewithFound', 'len', 'foundCVEs','intersectionCVEs'])
    for news in attack_cve_just_related_dict:
        print (list(attack_cve_related_dict[news]))
        # if count == 4:
        #     break
        ...
        CVEs_related_using_entity1 = []
        CVEs_related_using_entity2 = []
        CVEs_related_using_entity3 = []
        CVEs_related_using_entity4 = []
        CVEs_related_using_entity5 = []        
        CVEs_related_using_entity6 = []
        CVEs_related_using_entity7 = []
        CVEs_related_using_entity8 = []
        CVEs_related_using_entity9 = []
        found_cve_ids = list(set(attack_cve_related_dict[news]))
        intersectionCVEs = set(CVEs_id).intersection(set(found_cve_ids))
        # found_Cve_descriptions = get_description_found_cve(found_cve_ids)
        
        # attack_texts = " ".join(found_Cve_descriptions)
        attack_text = intersectionCVEs
        if len(attack_text) == 0:
            intersectionCVEs = found_cve_ids
        if len(intersectionCVEs) == 0:
            dataframeResults = pd.concat([dataframeResults, pd.DataFrame({'Attack':[news], 'CVEs':[sorted(set(CVEs_id))],'CVEsScorewithFoundFistCVE': [sorted(set(CVEs_related_using_entity))], 'len': [len(CVEs_related_using_entity)],'foundCVEs': [found_cve_ids], 'intersectionCVEs': [intersectionCVEs]})], ignore_index=True)
            continue
        attack_texts = "".join(list(intersectionCVEs)[0])
        attack_texts = get_description_found_cve([attack_texts])
        results = []
        external_embedding = model.encode(attack_texts)
        similarities = cosine_similarity(external_embedding.reshape(1, -1), embeddings)[0] 
        top_indices1 = np.argsort(similarities)[-181000:][::-1]
        ...
        for idx in top_indices1:
            score = similarities[idx]  # Get the precomputed similarity score
            CVEs_related_using_entity1.append(f'{CVEs_id[idx]}#{score:.2f}')
            print(f"CVE {CVEs_id[idx]}:")
            print("Description:", descriptions[idx])
            print("Score:", score)
            print("-" * 40)

        if len(found_cve_ids) > 1:
            attack_texts = "".join(found_cve_ids[1])
            attack_texts = get_description_found_cve([attack_texts])
            results = []
            external_embedding = model.encode(attack_texts)
            similarities = cosine_similarity(external_embedding.reshape(1, -1), embeddings)[0] 
            top_indices2 = np.argsort(similarities)[-181000:][::-1]
            for idx in top_indices2:
                score = similarities[idx]  # Get the precomputed similarity score
                CVEs_related_using_entity2.append(f'{CVEs_id[idx]}#{score:.2f}')
                print(f"CVE {CVEs_id[idx]}:")
                print("Description:", descriptions[idx])
                print("Score:", score)
                print("-" * 40)

        if len(found_cve_ids) > 2:
            attack_texts = "".join(found_cve_ids[2])
            attack_texts = get_description_found_cve([attack_texts])
            results = []            
            external_embedding = model.encode(attack_texts)
            similarities = cosine_similarity(external_embedding.reshape(1, -1), embeddings)[0] 
            top_indices3 = np.argsort(similarities)[-181000:][::-1]
            for idx in top_indices3:
                score = similarities[idx]  # Get the precomputed similarity score
                CVEs_related_using_entity3.append(f'{CVEs_id[idx]}#{score:.2f}')
                print(f"CVE {CVEs_id[idx]}:")
                print("Description:", descriptions[idx])
                print("Score:", score)
                print("-" * 40)

        if len(found_cve_ids) > 3:
            attack_texts = "".join(found_cve_ids[3])
            attack_texts = get_description_found_cve([attack_texts])
            results = []            
            external_embedding = model.encode(attack_texts)
            similarities = cosine_similarity(external_embedding.reshape(1, -1), embeddings)[0] 
            top_indices4 = np.argsort(similarities)[-181000:][::-1]
            for idx in top_indices4:
                score = similarities[idx]  # Get the precomputed similarity score
                CVEs_related_using_entity4.append(f'{CVEs_id[idx]}#{score:.2f}')
                print(f"CVE {CVEs_id[idx]}:")
                print("Description:", descriptions[idx])
                print("Score:", score)
                print("-" * 40)
        if len(found_cve_ids) > 4:
            attack_texts = "".join(found_cve_ids[4])
            attack_texts = get_description_found_cve([attack_texts])
            results = []            
            external_embedding = model.encode(attack_texts)
            similarities = cosine_similarity(external_embedding.reshape(1, -1), embeddings)[0] 
            top_indices5 = np.argsort(similarities)[-181000:][::-1]
            for idx in top_indices5:
                score = similarities[idx]  # Get the precomputed similarity score
                CVEs_related_using_entity5.append(f'{CVEs_id[idx]}#{score:.2f}')
                print(f"CVE {CVEs_id[idx]}:")
                print("Description:", descriptions[idx])
                print("Score:", score)
                print("-" * 40)

        if len(found_cve_ids) > 5:
            attack_texts = "".join(found_cve_ids[5])
            attack_texts = get_description_found_cve([attack_texts])
            results = []            
            external_embedding = model.encode(attack_texts)
            similarities = cosine_similarity(external_embedding.reshape(1, -1), embeddings)[0] 
            top_indices6 = np.argsort(similarities)[-181000:][::-1]
            for idx in top_indices6:
                score = similarities[idx]  # Get the precomputed similarity score
                CVEs_related_using_entity6.append(f'{CVEs_id[idx]}#{score:.2f}')
                print(f"CVE {CVEs_id[idx]}:")
                print("Description:", descriptions[idx])
                print("Score:", score)
                print("-" * 40)

        if len(found_cve_ids) > 6:
            attack_texts = "".join(found_cve_ids[6])      
            attack_texts = get_description_found_cve([attack_texts])
            results = []            
            external_embedding = model.encode(attack_texts)
            similarities = cosine_similarity(external_embedding.reshape(1, -1), embeddings)[0] 
            top_indices7 = np.argsort(similarities)[-181000:][::-1]
            for idx in top_indices7:
                score = similarities[idx]  # Get the precomputed similarity score
                CVEs_related_using_entity7.append(f'{CVEs_id[idx]}#{score:.2f}')
                print(f"CVE {CVEs_id[idx]}:")
                print("Description:", descriptions[idx])
                print("Score:", score)
                print("-" * 40)

        if len(found_cve_ids) > 7:
            attack_texts = "".join(found_cve_ids[7])
            attack_texts = get_description_found_cve([attack_texts])
            results = []            
            external_embedding = model.encode(attack_texts)
            similarities = cosine_similarity(external_embedding.reshape(1, -1), embeddings)[0]
            top_indices8 = np.argsort(similarities)[-181000:][::-1]
            for idx in top_indices8:
                score = similarities[idx]  # Get the precomputed similarity score
                CVEs_related_using_entity8.append(f'{CVEs_id[idx]}#{score:.2f}')
                print(f"CVE {CVEs_id[idx]}:")
                print("Description:", descriptions[idx])
                print("Score:", score)
                print("-" * 40)

        if len(found_cve_ids) > 8:
            attack_texts = "".join(found_cve_ids[8])
            attack_texts = get_description_found_cve([attack_texts])
            results = []            
            external_embedding = model.encode(attack_texts)
            similarities = cosine_similarity(external_embedding.reshape(1, -1), embeddings)[0]
            top_indices9 = np.argsort(similarities)[-181000:][::-1]
            for idx in top_indices9:
                score = similarities[idx]  # Get the precomputed similarity score
                CVEs_related_using_entity9.append(f'{CVEs_id[idx]}#{score:.2f}')
                print(f"CVE {CVEs_id[idx]}:")
                print("Description:", descriptions[idx])
                print("Score:", score)
                print("-" * 40)        

        CVEs_related_using_entity = CVEs_related_using_entity1 + CVEs_related_using_entity2 + CVEs_related_using_entity3 + CVEs_related_using_entity4 + CVEs_related_using_entity5 + CVEs_related_using_entity6 + CVEs_related_using_entity7 + CVEs_related_using_entity8 + CVEs_related_using_entity9
    ...
    dataframeResults.to_excel(f"./Results/NewsResults/News/latest/NewsCVEsScoreWithFoundbetweenCVESwithJustOneCVE.xlsx", index=False)



def check_cves_found_in_news_to_related_all_found_cves(attack_cve_just_related_dict, attack_cve_related_dict):
    count = 0
    dataframeResults = pd.DataFrame(columns=['Attack','CVEs','CVEsScorewithFoundbetweenCVESwithAllFoundCVEs'])
    for news in attack_cve_just_related_dict:
        print (list(attack_cve_related_dict[news]))
        # if count == 4:
        #     break
        ...
        for idx in top_indices1:
            ...
            print("Score:", score)
            print("-" * 40)
        dataframeResults = pd.concat([dataframeResults, pd.DataFrame({'Attack':[news], 'CVEs':[sorted(set(CVEs_id))],'CVEsScorewithFoundbetweenCVESwithAllFoundCVEs': [sorted(set(CVEs_related_using_entity))]})], ignore_index=True)
        
        
    dataframeResults.to_excel(f"./Results/NewsResults/News/latest/NewsCVEsScoreWithFoundbetweenCVESwithAllFoundCVE.xlsx", index=False)


def check_within_attack_cves_entity(attack_cve_just_related_dict, attack_cve_related_dict):
    count = 0
    dataframeResults = pd.DataFrame(columns=['Attack','CVEs','CVEsScorewithFoundEntity'])
    for news in attack_cve_just_related_dict:
        print (list(attack_cve_related_dict[news]))
        # if count == 4:
        #     break
        ...
        results = []
        for cve_description in descriptions:
            is_valid, score = validate_within_cve_news_pair_entity(cve_description, attack_texts)
            results.append((cve_description, is_valid, score))
        ...
        found_cve_ids = list(set(attack_cve_related_dict[news]))
        intersectionCVEs = set(CVEs_id).intersection(set(found_cve_ids))
        dataframeResults = pd.concat([dataframeResults, pd.DataFrame({'Attack':[news], 'CVEs':[sorted(set(CVEs_id))],'CVEsScorewithFoundEntity': [sorted(set(CVEs_related_using_entity))]})], ignore_index=True)
        
        
    dataframeResults.to_excel(f"./Results/NewsResults/News/latest/NewsCVEsScoreWithUsingEntityAttackCVE.xlsx", index=False)
...
if __name__ == "__main__":
    ...
    # add_scores_to_related_cves(attack_cve_just_related_dict, found_cves_dict)
    # check_cves_found_in_news_to_related_fisrt_cves(attack_cve_just_related_dict, found_cves_dict)
    # check_cves_found_in_news_to_related_all_found_cves(attack_cve_just_related_dict, found_cves_dict)
    check_within_attack_cves_entity(attack_cve_just_related_dict, found_cves_dict)
    # results = check_between_the_existing_CVEs_using_entity2(attack_cve_just_related_dict, found_cves_dict)
