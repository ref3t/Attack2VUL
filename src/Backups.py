...
for infoData in informationData:
    ...
    for model_name in SentenceTransformersModels:
        ...
        print(model_name)
        df.to_excel(f"Results/Results{model_name}_Main.xlsx", index=False)
        dfRes.to_excel(f"Results/Results{model_name}_Details.xlsx", index=False)
        dataframeResultsForallModels.to_excel(f"Results/AllModelsNFineTuned.xlsx", index=False)
