...
def read_Capec_negative(file_path, numberofPositive):
...
negative_samples = read_Capec_negative(negative_dataset_path, unique_positive_capecs)
...
while len(negative_pairs) < len(positive_pairs):
    ...
    if similarity < 0.2 and pair not in seen_pairs:
...
df_balanced = pd.DataFrame(balanced_data, columns=['CAPECID', 'CAPECName', 'CAPECDescription', 'CVEID', 'CVEDescription', 'Label'])

# Save to Excel
df_balanced.to_excel('dataset/attackInfo/CAPECs/Capec_data_Balanced.xlsx', index=False)
...
# Save to Excel files
train_data.to_excel('dataset/attackInfo/CAPECs/Capec_train_data_Balanced.xlsx', index=False)
val_data.to_excel('dataset/attackInfo/CAPECs/Capec_val_data_Balanced.xlsx', index=False)
test_data.to_excel('dataset/attackInfo/CAPECs/Capec_test_data_Balanced.xlsx', index=False)
