...
negative_dataset_path = "./FinalResultSeperate/CAPEC/CAPECNegatives.xlsx"  # Path to negative dataset
...
unique_positive_techniques = positive_df['CAPECID'].nunique()
...
for _, row in positive_df.iterrows():
    positive_pairs.append((row['CAPECID'], row['CAPECName'], row['CAPECDescription'], row['CVEID'], row['CVEDescription'], 1))  # Label as 1
...
train_data.to_excel('Capec_train_data.xlsx', index=False)
val_data.to_excel('Capec_val_data.xlsx', index=False)
test_data.to_excel('Capec_test_data.xlsx', index=False)
