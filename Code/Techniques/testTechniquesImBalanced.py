...
def trueNegativeSUM(vul_data_array,cve_ids_A_not_attack,Threeshold):
    """
    Calculates the count of true negatives in a dataset of vulnerabilities.

    Args:
        vul_data_array (list): A list of vulnerability data objects, each containing
            a CVE_ID and CVE_Smiliraty attribute.
        cve_ids_A_not_attack (list): A list of CVE IDs that are not associated with attacks.
        Threeshold (float): The similarity threshold below which a CVE is considered 
            not significant.

    Returns:
        tuple: A tuple containing two counts:
            - count: Number of CVEs in vul_data_array with a similarity less than the 
              threshold and present in cve_ids_A_not_attack.
            - count2: Number of CVEs in cve_ids_A_not_attack that are not present in
              vul_data_array.
    """
    count = 0
...
def readAllTech():
    ...
    if not os.path.exists(file_pathPositive):
        print("Error: File not found!")
        return {}

    # Read the Excel file
    data = pd.read_excel(file_pathPositive, header=0)
    ...
    # Drop NaN values in 'TechniqueID'
    data = data.dropna(subset=['TechniqueID'])

    # Check unique values in 'TechniqueID'
    print("Unique TechniqueID values:", data['TechniqueID'].unique())
    ...
    for mod in SentenceTransformersModels:
        ...
        for key, value in test_data.items():
            ...
            if infoData == "CAPEC":
                ...
                trainAndTestSet = dataCVE[dataCVE['TechniqueID']== key]
            ...
            if infoData == "CAPEC":
                ...
                CvesNotAttack = dataCVE[dataCVE['TechniqueID']!=key]
        ...
        dataframeResults.to_excel(f"Results/AllModelsResultsImBalanced.xlsx", index=False)
