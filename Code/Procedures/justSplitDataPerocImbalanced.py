...
def read_tech_negative(file_path):
    ...
    cleaned_data = data[['ProceduresID', 'ProcedureDescription']].dropna()
    return cleaned_data
...
unique_positive_techniques = positive_df['ProceduresID'].nunique()
...
positive_df = positive_df.drop_duplicates(subset=['ProceduresID', 'ProcedureDescription', 'CVEID', 'CVEDescription', 'Label'])
...
final_df = pd.DataFrame(balanced_data, columns=['ProceduresID', 'ProcedureDescription', 'CVEID', 'CVEDescription', 'Label'])
final_df.to_excel('dataset/attackInfo/Procedures/Proc_data_ImBalanced.xlsx', index=False)
# Train-test split
